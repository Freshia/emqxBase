#!/bin/sh
cd emqx/emqx-rel/_build/emqx/rel/emqx/
./bin/emqx start
...
WAIT_TIME=0
EMQX_WAIT_TIME=5
while [[ -z "$(./bin/emqx_ctl status |grep 'is running'|awk '{print $1}')" ]]
do
    sleep 1
...
do  
    ...
    if [[ ! -z "$(./bin/emqx_ctl status |grep 'is running'|awk '{print $1}')" ]]; then
        IDLE_TIME=0
        # Print the latest emqx.log
        if [[ -f emqx/emqx-rel/_build/emqx/rel/emqx/log/emqx.log.${NEW_LOG_NUM} ]];then
            tail -f emqx/emqx-rel/_build/emqx/rel/emqx/log/emqx.log.${NEW_LOG_NUM} &
            [[ ! -z $(ps -ef |grep "tail -f ./log/emqx.log" | grep -vE "grep|emqx.log.${NEW_LOG_NUM}" | awk '{print $1}') ]] && kill $(ps -ef |grep "tail -f ./log/emqx.log" | grep -vE "grep|emqx.log.${NEW_LOG_NUM}" | awk '{print $1}') 
...
# tail $(ls emqx/emqx-rel/_build/emqx/rel/emqx/log/*)
